...
# txt2pdf -- Text to PDF
#
# by SANFACE Software <sanface@sanface.com> 25th January 2017
#
# txt2pdf 11.x is shareware.
# $59 for the Personal Edition
# This means that you use the software for Personal purpose (home user, not
# inside a company)
# 
# This is the price for the Enterprise Edition:
# $150 (US) for 1 core computer 
# + $5 for every phisical core
# e.g. 6 phisical core server fee is: $150 + 5 x $5 = $175
# or
# $200 (US) for 1 core virtual computer
# + $5 for every virtual core
#
# You can upgrade txt2pdf 8.x, 9.x, 10.x to the new version with the discount of
...
#
# Try it for 30 days.
# If you decide to continue using it, register NOW
# your PE (Personal Edition, only for a personal use) or
# your EE (Enterprise Edition, for a use inside a company) at
# http://www.sanface.com/txt2pdf.html#register
# or contact SANFACE Software sanface@sanface.com
#
# This is version 11.1
#
# use strict;
use Getopt::Long;
use File::Basename;
...
my $SECURESERVER="http://www.sanface.com/txt2pdf.html#register";
...
my @elem=("tmpdir","author","creator","keywords","subject","title","landscape",
          "paper","defaultfont","usedfonts","lines","tab","pointSize",
	  "vertSpace","transition","npage","colour","fontmark","beginfile",
	  "endfile","border","bgdesign","fgdesign","pagemode","pagelayout",
	  "rotate","typeencoding","withextension","sleep","pdfdir","txtdir",
	  "viewerpreferences","fit","list","zoom","inputmod","outputmod",
	  "pdfversion","annotationtext","fontfile","linkuse","noid",
	  "pagemark","underline","prepdf");
my %option=(tmpdir             => './',
            author             => '',
            creator            => '',
            keywords           => '',
...
$lines = int $lines;
if  ($option{'defaultfont'} =~ /F(\d+)/)
  {
  $fontn=$1;
  if($option{'usedfonts'})
    {$option{'usedfonts'}.=",".$option{'defaultfont'}}
  else {$option{'usedfonts'}=$option{'defaultfont'}}
  }
else  {
  &Warning("the default font $option{'defaultfont'} isn't a Fxx font\n$producer1 will use the default font F1");
  if(!$option{'usedfonts'}) {$option{'usedfonts'}="F1"}
  $fontn=1;
  }
  $option{'usedfonts'}.=",";
  $verbose and print "final usedfonts $option{'usedfonts'}\n";
...
sub WriteHeader {
  ...
  $info=$obj;
    ...
    &pos("/Producer($producer1 v$version \\251 $companyname)");
    &pos("/Title($title)");
...
# Japanese HeiseiKakuGo
  if  ($option{'usedfonts'} =~ /F24,/)
    {
    $location[++$obj]=$fpos;
...
# Traditionl Chinese PMingLiU
  if  ($option{'usedfonts'} =~ /F30,/)
    {
    $location[++$obj]=$fpos;
...
# Simplified Chinese STSong
  if  ($option{'usedfonts'} =~ /F35,/)
    {
    $location[++$obj]=$fpos;
...
# Korean HYSMyeongJoStd
  if  ($option{'usedfonts'} =~ /F40,/)
    {
    $location[++$obj]=$fpos;
...
TXT2PDF - Version 11.1 25th January 2017
