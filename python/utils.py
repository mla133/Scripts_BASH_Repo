...
##################################
# floating-point function (double)
##################################
def decode_ieee_d(val_int):
...
def encode_ieee_d(val_double):
    """Encode Python double to int (64 bits integer) as an IEEE double precision

        Support NaN.

        :param val_float: double value to convert
        :type val_float: double
        :returns: IEEE 64 bits (single precision) as Python int
        :rtype: int
    """
    return struct.unpack("Q", struct.pack("d", val_double))[0]
